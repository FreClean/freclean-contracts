# Hotel Cleaning Services Agreement

**Template — requires legal review before use.** This is a working draft, consistent with FreClean's realism principle: it is not represented as final legal text until reviewed by counsel.

---

**This Hotel Cleaning Services Agreement** ("Agreement") is entered into as of ______________ ("Effective Date"), by and between:

**FreClean**, with operations in Léogâne, Ouest, Haiti ("Service Provider"), and

**______________________________** , a hotel property located at ______________________________ ("Client").

## 1. Term

This Agreement begins on the Effective Date and continues for a term of **☐ 2 years ☐ 3 years ☐ 4 years** (select one; "Initial Term"), unless terminated earlier under Section 8. Unless either party gives written notice of non-renewal at least 60 days before the end of the Initial Term, this Agreement automatically renews for successive one-year terms.

## 2. Services

Service Provider will perform room turnover cleaning, and common-area cleaning if selected below, following the checklist standard maintained in FreClean's operations documentation (available to Client on request):

- ☐ Room turnover cleaning (checked-out rooms, per the frequency in Schedule A)
- ☐ Common area cleaning (lobby, hallways, elevators)
- ☐ Additional services: ______________________________

## 3. Schedule & Volume

Room and common-area volume, frequency, and staffing levels are set out in **Schedule A**, attached and incorporated by reference. Schedule A may be updated by mutual written agreement without amending this Agreement.

## 4. Fees & Payment

- Fees are set out in **Schedule B**.
- Client may pay by: cash, international card, or a supported Celo stablecoin via FreClean's Web3 payment system (CeloHT) — see FreClean's payment policy for current status of each method.
- Invoices are issued **☐ monthly ☐ quarterly** and due within 15 days of issuance.
- A payment record for this Agreement, and confirmation of each payment, may be recorded in FreClean's `freclean-contracts` agreement registry on the Celo network — this is a record-keeping measure only and does not change either party's payment obligations under this Agreement.

## 5. Quality Standards

Service Provider follows its documented room-ready checklist and end-of-job verification standard for every serviced room. Client may request a joint inspection at a mutually convenient time. Persistent quality issues are addressed under Section 9 (Dispute Resolution) before either party considers termination for cause.

## 6. Staffing

Service Provider is responsible for hiring, training, and supervising its own cleaning staff. Service Provider's staff are not employees of Client. Service Provider will ensure staff assigned to Client's property have undergone FreClean's standard training (see FreClean's operations documentation).

## 7. Insurance & Liability

- Service Provider maintains ☐ general liability ☐ other: ______________ insurance, evidence of which is available on request. *(Confirm actual coverage before use — not provided as a default here.)*
- Each party's liability to the other under this Agreement is limited to direct damages; neither party is liable for indirect, incidental, or consequential damages, except for breaches of confidentiality (Section 10) or willful misconduct.

## 8. Termination

- **For convenience:** either party may terminate with 90 days' written notice.
- **For cause:** either party may terminate immediately on written notice if the other party materially breaches this Agreement and fails to cure within 30 days of written notice of the breach.
- On termination, Client pays for all services performed through the termination date.

## 9. Dispute Resolution

The parties will attempt to resolve any dispute through good-faith discussion between designated representatives before pursuing formal action. *(Arbitration/litigation venue and governing law: not provided — to be finalized with counsel.)*

## 10. Confidentiality

Each party will keep the other's non-public business information confidential and use it only to perform this Agreement.

## 11. Force Majeure

Neither party is liable for a failure to perform caused by events beyond its reasonable control (natural disaster, civil unrest, government action), provided the affected party gives prompt notice and resumes performance as soon as reasonably possible.

## 12. General

- This Agreement, with its Schedules, is the entire agreement between the parties on this subject.
- Amendments must be in writing and signed by both parties.
- Neither party may assign this Agreement without the other's written consent, except to a successor in a merger or acquisition.
- Governing law: ______________________________ *(not provided — to be finalized with counsel).*

---

**Signatures**

FreClean: _______________________  Date: __________
Name/Title: _______________________

Client: _______________________  Date: __________
Name/Title: _______________________

---

**Schedule A — Volume & Frequency:** *(to be completed per property)*

**Schedule B — Fees:** *(to be completed per property — not provided as a default rate here; see FreClean's pricing guidance)*
